...
void MainPlayer::tick(std::vector<Sprite*> sprites) {
    if(isMoving)
        move(); //innebär att spelaren har tryckt ner en av piltangenterna och karaktären ska flyttas enligt det
    isMoving = false;
...
MainPlayer::MainPlayer(int x, int y, int w, int h, int s, int l): MoveableByKeysSprite(x, y, w, h, s, DOWN, l) {
    ...
    downSurface = IMG_Load("/Users/paulinakekkonen/Pictures/GameImages/characterDown.png");
    Uint32 dWhite = SDL_MapRGB(downSurface->format, 255, 255, 255);
    SDL_SetColorKey(downSurface, true, dWhite);
    upSurface = IMG_Load("/Users/paulinakekkonen/Pictures/GameImages/characterUp.png");
    Uint32 uWhite = SDL_MapRGB(upSurface->format, 255, 255, 255);
    SDL_SetColorKey(upSurface, true, uWhite);
    leftSurface = IMG_Load("/Users/paulinakekkonen/Pictures/GameImages/characterLeft.png");
    Uint32 lWhite = SDL_MapRGB(leftSurface
                               ->format, 255, 255, 255);
    SDL_SetColorKey(leftSurface, true, lWhite);
    rightSurface = IMG_Load("/Users/paulinakekkonen/Pictures/GameImages/characterRight.png");
